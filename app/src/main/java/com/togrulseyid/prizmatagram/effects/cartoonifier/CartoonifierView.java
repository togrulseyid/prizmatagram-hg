package com.togrulseyid.prizmatagram.effects.cartoonifier;
...
//        Notification notification = new Notification(R.drawable.ic_cancel, "Saving to gallery (image " + mNotificationID + ") ...", System.currentTimeMillis());
    ...
    protected Bitmap processFrame(byte[] data) {
        ...
        if (!bSaveThisFrame) {
            ...
            if (m_sketchMode) {
                ...
                NativeClass.cartoonifyImage(getFrameWidth(), getFrameHeight(), data, rgba, m_sketchMode, m_alienMode, m_evilMode, m_debugMode);
            } else {
                // Just prepare the camera image for display without any modification. Uses C/C++ code through NDK (JNI).
                NativeClass.ShowPreview(getFrameWidth(), getFrameHeight(), data, rgba);
            }
        } else {
            // Just do it once.
