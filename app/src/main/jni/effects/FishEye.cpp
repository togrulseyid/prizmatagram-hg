...
void FishEye::process()
{

	cv::Mat imgOut = mat->clone();
//    fishEye(*mat, imgOut, (*mat).cols / 2, (*mat).rows / 2, 0.0001);
    fishEye(imgOut, *mat, imgOut.cols / 2, imgOut.rows / 2, 0.0001);
...
//void FishEye::fishEye(InputArray _src, OutputArray _dst, double Cx, double Cy, double k, bool scale = true)
void FishEye::fishEye(InputArray _src, OutputArray _dst, double Cx, double Cy, double k)
{
    bool scale = true;
